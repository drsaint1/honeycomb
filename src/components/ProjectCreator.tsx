...
export const ProjectCreator: React.FC = () => {
  ...
  const [assemblerConfigAddress, setAssemblerConfigAddress] = useState<
    string | null
  >(null);
  const [charactersTreeAddress, setCharactersTreeAddress] = useState<
    string | null
  >(null);
  ...
  const [xpUpdateStatus, setXpUpdateStatus] = useState<string>("");
  ...
  const createNFTInfrastructure = async (projectAddr: string) => {
    try {
      ...
      console.log(
        "🔧 FIXED: Using unique ticker and exact parameter format from docs"
      );
      ...
      try {
        ...
        console.error(
          "❌ createCreateAssemblerConfigTransaction failed:",
          createError
        );
        ...
        if (
          ...
          createError.message.includes(
            "already exists, try fetching using address:"
          )
        ) {
          const addressMatch = createError.message.match(
            /address: ([A-Za-z0-9]+)/
          );
          if (addressMatch) {
            const existingAddress = addressMatch[1];
            console.log(
              "✅ Assembler config already exists at:",
              existingAddress
            );
            ...
            console.log(
              "🔄 Skipping assembler config creation, proceeding to character model..."
            );
            // Don't throw error, continue to character model creation
          } else {
            throw new Error(
              `Failed to create assembler config transaction: ${createError.message}`
            );
          }
        } else {
          throw new Error(
      ...
      if (assemblerResponse) {
        ...
        // Extract the actual transaction from the response
        const assemblerTransaction =
          assemblerResponse?.createCreateAssemblerConfigTransaction ||
          assemblerResponse;
        console.log(
          "🔍 Extracted assembler transaction:",
          assemblerTransaction
        );
        console.log("🔍 Transaction type:", typeof assemblerTransaction);
        console.log(
          "🔍 Transaction keys:",
          assemblerTransaction
            ? Object.keys(assemblerTransaction)
            : "null/undefined"
        );
        ...
        console.log("  - transaction structure:", {
          ...
          keys: assemblerTransaction
            ? Object.keys(assemblerTransaction)
            : "none",
          isArray: Array.isArray(assemblerTransaction),
          stringified:
            JSON.stringify(assemblerTransaction, null, 2).substring(0, 500) +
            "...",
        });
        ...
        if (
          assemblerResponse &&
          typeof assemblerResponse === "object" &&
          "createCreateAssemblerConfigTransaction" in assemblerResponse
        ) {
          transactionToSend =
            assemblerResponse.createCreateAssemblerConfigTransaction;
        ...
        if (
          transactionToSend &&
          typeof transactionToSend === "object" &&
          "tx" in transactionToSend
        ) {
        ...
        try {
          ...
          console.log(
            "  - transaction keys:",
            finalTransaction ? Object.keys(finalTransaction) : "null"
          );
          console.log("  - wallet connected:", walletWrapper.connected);
          console.log(
            "  - wallet pubkey:",
            walletWrapper.publicKey?.toString()
          );
          ...
          console.log(
            "✅ sendClientTransactions succeeded for assembler config"
          );
          console.log("🔍 Returned signatures:", assemblerSignatures);
          console.log(
            "🔍 Signature types:",
            assemblerSignatures
              ? assemblerSignatures.map((s) => typeof s)
              : "none"
          );
          ...
          throw new Error(
            `Failed to send assembler transaction: ${sendError.message}`
          );
        }
        ...
        if (assemblerSignatures && assemblerSignatures.length > 0) {
          ...
          console.log(
            "🔍 Full assembler response for debugging:",
            assemblerResponse
          );
          ...
          console.log("🔍 Assembler transaction details:", {
            ...
            transactionKeys: assemblerTransaction
              ? Object.keys(assemblerTransaction)
              : "none",
            responseKeys: assemblerResponse
              ? Object.keys(assemblerResponse)
              : "none",
          });

          // Try to extract the assembler config address from the transaction response directly
          console.log(
            "🔍 Trying to extract assembler config address from transaction response..."
          );
          ...
          if (assemblerResponse?.assemblerConfig) {
            ...
            console.log(
              "🔍 Method 1: Found assembler config in response:",
              assemblerAddr
            );
          }
          ...
          if (!assemblerAddr && assemblerTransaction?.assemblerConfig) {
            ...
            console.log(
              "🔍 Method 2: Found assembler config in transaction:",
              assemblerAddr
            );
          }
          ...
          if (
            ...
            assemblerAddr =
              assemblerResponse.createCreateAssemblerConfigTransaction
                .assemblerConfig;
            console.log(
              "🔍 Method 3: Found assembler config in nested response:",
              assemblerAddr
            );
          }
          ...
          if (!assemblerAddr && typeof assemblerSignatures[0] === "string") {
            ...
            console.log(
              "🔍 Method 4: Using transaction signature as address:",
              assemblerAddr
            );
          }
          ...
          if (assemblerAddr && assemblerAddr !== "UNKNOWN") {
            ...
            console.log(
              "✅ Successfully determined assembler config address:",
              assemblerAddr
            );
          } else {
            console.log(
              "🔍 Could not determine address from response, trying blockchain query..."
            );
            ...
            try {
              console.log(
                "🔍 Querying assembler configs with different formats..."
              );
              ...
              console.error(
                "❌ All assembler config queries failed:",
                generalError
              );
            }
          }
          ...
          if (!assemblerAddr || assemblerAddr === "UNKNOWN") {
            ...
            throw new Error(
              "Failed to create assembler config: address could not be determined"
            );
          }
          ...
          console.log(
            "ℹ️ Skipping blockchain verification due to indexing delays - proceeding with known address"
          );
        ...
        console.log(
          "✅ Using existing assembler config, skipping transaction processing"
        );
        // assemblerAddr should already be set from the catch block
        if (assemblerAddr) {
          console.log(
            "✅ Using existing assembler config address:",
            assemblerAddr
          );
        } else {
          throw new Error(
            "Assembler config address not found despite indicating it exists"
          );
        }
      }
      ...
      const modelResponse = await client.createCreateCharacterModelTransaction(
        {
          ...
          config: {
            ...
            assemblerConfigInput: {
              ...
              symbol: "RCAR",
              description:
                "High-performance racing cars for the Honeycomb Racing Game",
      ...
      const modelTransaction =
        modelResponse?.createCreateCharacterModelTransaction || modelResponse;
      ...
      if (modelSignatures && modelSignatures.length > 0) {
        ...
        try {
          ...
          const characterMethods = clientMethods.filter(
            (method) =>
              method.toLowerCase().includes("character") &&
              method.toLowerCase().includes("model")
          );
          console.log("🔍 Character model related methods:", characterMethods);

          const allFindMethods = clientMethods.filter((method) =>
          ...
          if (
            ...
            actualCharacterModels.characterModel.length > 0
          ) {
          ...
          if (!modelAddr) {
            ...
            console.log(
              "🔍 Searching for any character models for this project..."
            );
            try {
              const anyModels = await client.findCharacterModels({
                project: projectAddr,
              });
        ...
        if (modelAddr && modelAddr !== "FAILED_TO_CREATE") {
          ...
          try {
            ...
            console.log(
              "🔍 Character model verification result:",
              modelVerification
            );

            const modelExists = modelVerification?.characterModel?.some(
              (model: any) =>
                model.address === modelAddr ||
                model.id === modelAddr ||
                model.publicKey === modelAddr
            );

            if (!modelExists) {
              console.error(
                "❌ CRITICAL: Character model not persisted on blockchain!"
              );
              console.log("🔍 Expected address:", modelAddr);
              console.log(
                "🔍 Found models:",
                modelVerification?.characterModel?.map(
                  (m: any) => m.address || m.id || m.publicKey
                )
              );
              throw new Error(
                "Character model transaction succeeded but model not found on blockchain"
              );
            }

            console.log(
              "✅ Character model verified and persisted on blockchain"
            );
          } catch (verificationError: any) {
            console.error(
              "❌ Character model verification failed:",
              verificationError
            );
            throw new Error(
              `Character model verification failed: ${verificationError.message}`
            );
          }
        }
      } else {
        throw new Error(
          "No character model signatures returned from transaction"
        );
      }
      ...
      const charactersTreeTransaction =
        treeResponse?.createCreateCharactersTreeTransaction || treeResponse;
      ...
      if (treeSignatures && treeSignatures.length > 0) {
        ...
        throw new Error(
          "No characters tree signatures returned from transaction"
        );
      }
  ...
  // Add XP and achievements to player profile
  const updatePlayerXP = async (
    profileAddress: string,
    xpToAdd: number,
    achievementsToAdd: number[] = []
  ) => {
    if (
      !connected ||
      !publicKey ||
      ...
      setXpUpdateStatus("❌ Please connect your wallet first");
      return;
    }

    setIsUpdatingXP(true);
    setXpUpdateStatus("🎯 Adding XP and achievements...");
    ...
    try {
      ...
      console.log('🔍 Project address:', import.meta.env.VITE_PUBLIC_HONEYCOMB_PROJECT_ADDRESS);
      console.log('🔍 Profile address:', profileAddress);

      // Query the project to find the actual authority
      const projectQuery = await client.findProjects({
        addresses: [import.meta.env.VITE_PUBLIC_HONEYCOMB_PROJECT_ADDRESS]
      }, {
        fetchOptions: {
          headers: { authorization: `Bearer ${accessToken}` }
        }
      ...
      if (projectQuery?.project && projectQuery.project.length > 0) {
        ...
        console.log('🔍 Authority matches?', project.authority === publicKey.toString());
      }

      // Check if the profile belongs to this project
      const profileQuery = await client.findProfiles({
      ...
      if (profileQuery?.profile && profileQuery.profile.length > 0) {
        const profile = profileQuery.profile[0];
        console.log('🔍 Profile project:', profile.project);
        console.log('🔍 Current project:', import.meta.env.VITE_PUBLIC_HONEYCOMB_PROJECT_ADDRESS);
        console.log('🔍 Profile belongs to current project?', profile.project === import.meta.env.VITE_PUBLIC_HONEYCOMB_PROJECT_ADDRESS);
      }
      ...
      if (!txResponse) {
        throw new Error("No XP update transaction received from Honeycomb API");
      }
      ...
      if (signatures && signatures.length > 0) {
        const sigString = typeof signatures[0] === 'string' ? signatures[0] : JSON.stringify(signatures[0]);
        setXpUpdateStatus(
          `✅ Successfully added ${xpToAdd} XP! Transaction: ${sigString.substring(0, 8)}...`
        );
        console.log("✅ XP updated successfully:", signatures[0]);
      } else {
        throw new Error("XP update transaction failed");
      }
    } catch (error: any) {
      console.error("❌ XP update failed:", error);
      setXpUpdateStatus(`❌ Error: ${error.message}`);
    } finally {
      setIsUpdatingXP(false);
      // Clear status after 10 seconds
      setTimeout(() => setXpUpdateStatus(""), 10000);
    }
  };

  // Demo function to test XP system - adds XP directly to connected user's profile
  const testXPSystem = async () => {
    ...
    try {
      setXpUpdateStatus("🔍 Finding your profile...");
      ...
      if (!userProfiles?.profile || userProfiles.profile.length === 0) {
        setXpUpdateStatus("❌ No profile found. Create a profile first.");
        return;
      }
      ...
      setXpUpdateStatus(`❌ Error finding profile: ${error.message}`);
    }
  };
        ...
        >
          ...
          >
            ...
            {(characterModelAddress ||
              ...
              <div
                style={{ fontSize: "11px", color: "#2e7d32", marginTop: "5px" }}
              >
                {characterModelAddress && (
                  <div>
                    ✅ Character Model:{" "}
      ...
      {import.meta.env.VITE_PUBLIC_HONEYCOMB_PROJECT_ADDRESS && (
        ...
        >
          ...
          >
            ...
            >
              Test the Honeycomb platform data XP system (requires valid profile
              address)
            </p>

            <button
              onClick={testXPSystem}
              disabled={!connected || isUpdatingXP}
              style={{
                background: connected && !isUpdatingXP ? "#1976d2" : "#6c757d",
                color: "white",
                border: "none",
                padding: "8px 16px",
                borderRadius: "4px",
                cursor: connected && !isUpdatingXP ? "pointer" : "not-allowed",
                fontSize: "12px",
                fontWeight: "bold",
                marginRight: "10px",
              }}
            >
              {isUpdatingXP ? "🔄 Adding XP..." : "🎯 Test XP System"}
            </button>
            ...
            {xpUpdateStatus && (
              <div
                style={{
                  marginTop: "10px",
                  padding: "8px",
                  background: xpUpdateStatus.startsWith("✅")
                    ? "#d4edda"
                    : "#f8d7da",
                  color: xpUpdateStatus.startsWith("✅")
                    ? "#155724"
                    : "#721c24",
                  borderRadius: "4px",
                  fontSize: "11px",
